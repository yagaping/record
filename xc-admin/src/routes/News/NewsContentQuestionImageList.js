...
import NewsContentQuestionImageModal from "../../components/NewsContentQuestionImageModal";
...
@Form.create()
export default class NewsContentQuestionImageList extends PureComponent {
  state = {
    modalTitle: '标题',
  ...
  handleModalModify = (newValues, oldValue) => {
    // 修改需要注意：后台有 两属性不一致.
    this.props.dispatch({
      type: 'newsContentList/modifyNewsQuestionImage',
      payload: {
        body: {
          ...newValues,
          newsContentQuestionImageId: oldValue.newsContentQuestionImageId,
  ...
  handleModalAdd = (newValues) => {
    const { params } = this.props.match;
    this.props.dispatch({
      type: 'newsContentList/addQuestionImage',
      payload: {
        body: {
          ...newValues,
          newsContentQuestionId: params.newsId,
          triggerExpression: newValues.triggerValue,
        },
      },
      callback: () => {
