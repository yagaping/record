...
import NewsContentQuestionImageTable from '../../components/NewsContentQuestionImageTable';
import styles from './NewsList.less';
import NewsQuestionAnswerImageModal from "../../components/NewsQuestionAnswerImageModal";
import NewsQuestionAnswerImageTable from "../../components/NewsQuestionAnswerImageTable";
...
@Form.create()
export default class NewsQuestionAnswerImageList extends PureComponent {
  state = {
    modalTitle: '标题',
  ...
  queryContentList() {
    ...
    dispatch({
      type: 'newsContentList/queryQuestionAnswerImageList',
      payload: {
  ...
  handleFormReset = () => {
    ...
    const { params } = this.props.match;
    dispatch({
      type: 'newsContentList/queryQuestionImage',
      payload: {
        index: 0,
        newsId: params.newsId,
      },
  ...
  handleModalModify = (newValues, oldValue) => {
    // 修改需要注意：后台有 两属性不一致.
    this.props.dispatch({
      type: 'newsContentList/modifyNewsQuestionAnswerImage',
      payload: {
        body: {
          ...newValues,
          newsQuestionAnswerImageId: oldValue.newsQuestionAnswerImageId,
  ...
  handleModalAdd = (newValues) => {
    const { params } = this.props.match;
    this.props.dispatch({
      type: 'newsContentList/addQuestionAnswerImage',
      payload: {
        body: {
          ...newValues,
