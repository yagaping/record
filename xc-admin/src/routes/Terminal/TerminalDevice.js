...
export default class TerminalDevice extends PureComponent{
  state = {
      params:{
        beginDay:'',
        endDay:'',
  ...
  render(){
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if(activeKey == 1){
        ...
        obj.x = item.mblResolution
      }else if(activeKey == 3){
        let val
          if(item.mblSystem == 1){
            val = 'Android'
          }else if(item.mblSystem == 2){
            val = 'iOS'
          }
        obj.x = val
      }
    ...
    return (
      <PageHeaderLayout>
          ...
          <Tabs activeKey={activeKey} onChange={this.callback}>
            <TabPane tab="机型" key="1">
            </TabPane>
            <TabPane tab="分辨率" key="2">
            </TabPane>
