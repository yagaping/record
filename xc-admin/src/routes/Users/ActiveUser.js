...
export default class ActiveUser extends PureComponent{
          ...
          callback:(res)=>{
            if(res.code == 0 || res.code == 1){
              ...
              for(let i=result.length-1;i>=0;i--){
                ...
                obj.startCount = item.userActTotal;
                obj.dateTime = time;
                areaData.push(obj);
  ...
  render(){
    ...
    const { timeType, list, areaData } = this.state;
    const column = [
      {
        title:'日期',
        key:'dateTime',
        ...
        render:(key,row)=>{
          let val
          if(row.userNewTotal && row.userActTotal){
            val = parseInt(row.userNewTotal/row.userActTotal*100) + '%'
          }else{
            val = '0%';
          }
        ...
        title:'DAU/7日活跃用户',
        key:'userWkactTotal',
        dataIndex:'userWkactTotal',
        render:(key)=>{
          return <span>{key||'--'}</span>
        }
      },{
        title:'DAU/30日活跃用户',
        key:'userThreemthactTotal',
        dataIndex:'userThreemthactTotal',
        render:(key)=>{
          return <span>{key||'--'}</span>
        }
      }
    ...
    return (
      <PageHeaderLayout>
          ...
          <div className={styles.area}>
            <div className={styles.areaTitle}>活跃趋势</div>
            ...
            <AreaChart data={{areaData,title:'活跃用户'}}/>
