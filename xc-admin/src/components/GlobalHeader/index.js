...
import user_default from './user_default.png';
...
export default class GlobalHeader extends PureComponent {
  ...
  toggle = ( type ) => {
    const { collapsed, modStyle } = this.props;
    let number;
    if(collapsed){
      number = 256;
    }else{
          ...
          {currentUser.name ? (
            <Dropdown overlay={menu}>
              <span className={`${styles.action} ${styles.account}`}>
                <Avatar size="small" className={styles.avatar} src={currentUser.avatar||user_default} />
