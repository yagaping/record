...
export default class FestivalListNew extends Component {
  ...
  edit = (params) => {
    this.props.dispatch( routerRedux.push({
        pathname: '/discover/new-festival-edit',
        params: params,
      }
    ));
  }
  ...
  render() {
    ...
    const columns = [{
      ...
      key: 'pushTitle',
      render:(key)=>{
        return <div style={{minWidth:'150px'}}>{key}</div>
      }
    }, {
      title: '预提醒',
      ...
      key: 'perPushText',
      render: (value, row, index) => {
        return(<a key={index} href={row.jumpLink ? row.jumpLink : 'javascript:;'} target='blank' style={{minWidth:'100px',display:'block'}}>{value ? value : '暂无'}</a>)
      }
    }, {
      title: '提醒',
      dataIndex: 'pushText',
      ...
      <div>
        <div className={styles.tableList}>
          ...
          <Modal visible={this.state.imgVisible} footer={null} onCancel={this.imgCancel} width={360} style={{position:'relative'}}>
              <img src={this.state.imgArray}  width='320' />
          </Modal>
        </div>
      </div>   
    );
